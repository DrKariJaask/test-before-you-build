# Market Validation Services and Pricing

**Summary:** Independent market validation through 20 customer interviews, evidence analysis and a clear BUILD, CHANGE or KILL recommendation.

**Fixed project price:** €2,900 + VAT  
**Typical timeline:** Approximately 2–4 weeks  
**Canonical HTML:** https://testbeforeyoubuild.com/market-validation-services-pricing/

## Know whether your idea is worth building — before you build it

Dr. Kari defines the relevant potential-buyer or user group and identifies approximately 30–40 suitable prospects based on the agreed target definition. The client approves the target list before outreach begins. Dr. Kari handles the outreach and conducts the 20 interviews.

The client receives the interview transcripts, evidence analysis, a written report, recommended changes and a BUILD, CHANGE or KILL recommendation.

## Who the service is for

The service is designed for founders, corporate innovation teams, researchers, technology-transfer teams, EU-funded projects, accelerators and investors with an identifiable target group and a meaningful development, funding or commercialisation decision.

It is not a substitute for statistically representative quantitative research and is not intended to produce positive feedback for a decision already made.

## What the service includes

- Decision and target-group definition
- Approximately 30–40 suitable prospects identified for client approval
- Recruitment of 20 interview participants
- 20 independent customer interviews
- Interview transcripts
- Analysis of problems, current behaviour, demand signals and objections
- Willingness-to-change and willingness-to-pay signals
- Written report and recommended changes
- BUILD, CHANGE or KILL recommendation

Participants may be realistic potential buyers, users or both. The service does not imply that every participant is a confirmed buyer.

## Process

1. Define the commercial decision and target group.
2. Dr. Kari identifies approximately 30–40 suitable prospects and obtains client approval.
3. Dr. Kari handles the outreach and recruits 20 participants.
4. Dr. Kari conducts and transcribes 20 interviews.
5. Analyse recurring themes, disagreements, objections and demand signals.
6. Deliver the evidence and recommendation.

Twenty interviews provide focused qualitative evidence about the strength, nature and conditions of demand. The standard service is not a limit: larger projects can be scoped separately when the commercial question requires broader coverage.

## BUILD, CHANGE or KILL

- **BUILD:** The evidence supports continued investment, subject to the identified risks and assumptions.
- **CHANGE:** The opportunity may be meaningful, but the customer, problem, value proposition, positioning or solution should be revised.
- **KILL:** The evidence does not currently justify further investment in the direction tested.

## Price and schedule

The fixed project price is **€2,900 + VAT**. A typical project takes approximately **2–4 weeks**. The schedule depends primarily on the accessibility and specialisation of the target customer group and is confirmed before the project begins.

## Service provider

Dr. Kari Jääskeläinen is an entrepreneur, business leader and consultant whose work combines market validation, technology commercialization and practical business development.

He has completed around 200 commercialization and business development projects and co-founded ventures in MedTech, private higher education, wireless technology and artificial intelligence.

[Read about Dr. Kari’s experience](https://testbeforeyoubuild.com/about-kari-jaaskelainen/).

## Is the service limited to 20 interviews?

No. The standard €2,900 + VAT service includes 20 interviews. Projects involving multiple customer segments, buyer roles, countries, use cases or broader objectives can be scoped and priced separately.

[Read how many customer interviews are enough](https://testbeforeyoubuild.com/how-many-customer-interviews/).

[Read how to validate a business idea before you build](https://testbeforeyoubuild.com/how-to-validate-a-business-idea/).

## Contact

[Submit your idea](https://testbeforeyoubuild.com/market-validation-services-pricing/#contact).

Do not include trade secrets or sensitive personal data in your initial message.
